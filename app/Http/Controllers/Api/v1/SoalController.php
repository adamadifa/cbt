...
class SoalController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (in_array(intval($request->tipe_soal), [SoalConstant::TIPE_BENAR_SALAH, SoalConstant::TIPE_SETUJU_TIDAK])) {
                $data['layout'] = SoalConstant::LAYOUT_KEBAWAH_TABEL;
            }

            if (in_array(intval($request->tipe_soal), [SoalConstant::TIPE_MENJODOHKAN, SoalConstant::TIPE_MENGURUTKAN])) {
            ...
            if(in_array(intval($request->tipe_soal), [
                ...
                foreach($request->pilihan as $key => $pilihan) {
                    if(in_array(intval($request->tipe_soal), [SoalConstant::TIPE_PG,SoalConstant::TIPE_LISTENING])) {
                        $correct = $request->correct == $key ? '1' : '0';
                    }
                    else if(in_array(intval($request->tipe_soal), [SoalConstant::TIPE_PG_KOMPLEK,SoalConstant::TIPE_BENAR_SALAH])) {
                    ...
                    if (intval($request->tipe_soal) == SoalConstant::TIPE_MENJODOHKAN) {
                        $pair = [
                            "a"  => [
                                "id"    => "a".uniqid(),
                                "text"  => $pilihan["a"]
                    ...
                    array_push($data, [
                        ...
                        'correct'       => $correct,
                        'created_at'    => now()->addSeconds($intervall)
                    ]);
                    $intervall++;
                }
            ...
            if(in_array(intval($request->tipe_soal), [
                ...
                foreach($request->pilihan as $key=>$pilihan) {
                    ...
                    array_push($data, [
                        ...
                        'text_jawaban'  => $pilihan,
                        'correct'       => $correct,
                        'created_at'    => now()->addSeconds($intval)
                    ]);
                    $intval++;
                }
                DB::table('jawaban_soals')->insert($data);
